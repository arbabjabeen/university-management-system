...
public void actionPerformed(ActionEvent ae) {
    if (ae.getSource() == submit) {
        ...
        if (rollno == null || rollno.equals("") || date.equals("") || duration.equals("")) {
            JOptionPane.showMessageDialog(null, "All fields are required");
            return;
        }

        // Date format validation (simple)
        if (!date.matches("\\d{4}-\\d{2}-\\d{2}")) {
            JOptionPane.showMessageDialog(null, "Date format must be YYYY-MM-DD");
            return;
        }

        // Date validity check (days in month, leap year)
        try {
        ...
        try {
            ...
            String checkQuery = "select * from studentleave where rollno='" + rollno + "' and leave_date='" + date + "'";
